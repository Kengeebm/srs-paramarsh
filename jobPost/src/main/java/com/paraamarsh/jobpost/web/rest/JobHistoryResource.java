...
/**
 * REST controller for managing {@link com.paraamarsh.jobpost.domain.JobHistory}.
 */
@RestController
@RequestMapping("/api")
public class JobHistoryResource {

    private final Logger Logger = LoggerFactory.getLogger(JobHistoryResource.class);

    private static final String ENTITY_NAME = "jobPostJobHistory";
    ...
    public ResponseEntity<JobHistory> createJobHistory(@Valid @RequestBody JobHistory jobHistory) throws URISyntaxException {
        Logger.debug("REST request to save JobHistory : {}", jobHistory);
    ...
    public ResponseEntity<JobHistory> updateJobHistory(@Valid @RequestBody JobHistory jobHistory) throws URISyntaxException {
        Logger.debug("REST request to update JobHistory : {}", jobHistory);
        if (jobHistory.getId() == null) {
    ...
    public ResponseEntity<List<JobHistory>> getAllJobHistories(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
        Logger.debug("REST request to get a page of JobHistories");
    ...
    public ResponseEntity<JobHistory> getJobHistory(@PathVariable String id) {
        Logger.debug("REST request to get JobHistory : {}", id);
    ...
    public ResponseEntity<Void> deleteJobHistory(@PathVariable String id) {
        Logger.debug("REST request to delete JobHistory : {}", id);
